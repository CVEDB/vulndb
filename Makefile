TEMP_DIR = ./.tmp
IMAGE_NAME = ghcr.io/cvedb/vulndb
BIN_DIR = ./bin
...
# this is the python package version for vulndb, based off of the git state
...
bootstrap: $(TEMP_DIR)  ## Download and install all tooling dependencies
	curl -sSfL https://raw.githubusercontent.com/cvedb/chronicle/main/install.sh | sh -s -- -b $(TEMP_DIR)/ $(CHRONICLE_VERSION)
...
.PHONY: dev
dev:  ## Get a development shell with locally editable grype, grype-db, and vulndb repos
	@DEV_vulndb_BIN_DIR=$(ABS_BIN_DIR) .github/scripts/dev-shell.sh $(provider) $(providers)
...
check-dev-shell:
	@test -n "$$DEV_vulndb_SHELL" || (echo "$(RED)DEV_vulndb_SHELL is not set. Run 'make dev provider=\"...\"' first$(RESET)" && exit 1)
...
check-types: virtual-env-check  ## Run type checks (mypy)
	mypy --config-file ./pyproject.toml src/vulndb
...
unit: virtual-env-check  ## Run unit tests
	pytest --cov-report html --cov vulndb -v tests/unit/
